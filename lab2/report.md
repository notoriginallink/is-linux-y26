...
### 23. Убедиться, что монтирование удалось и скопировать в каталог любой файл
Создадим на клиенте файл hello.txt и скопируем его в каталог
```
echo "Hello, NFS!" >> hello.txt
cp hello.txt /var/remotenfs
```
На сервере попробуем прочитать файл `cat /mnt/vol01/hello.txt`
``` 
Hello, NFS!
```
Работает!
